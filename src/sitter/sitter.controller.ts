...
import { ListQueryRequestDto, SitterDto, SitterListResponseDto, SitterRequestDto } from '@petman/common';
...
export class SitterController {
  ...
  @Post('/')
  async create(@Body() body: SitterRequestDto, @UploadedFiles() images, @SelectedUserParam() selectedUser: User): Promise<SitterDto> {
    if (!images.length) {
      throw new BadRequestException();
    }
    body.images = map(images, image => join(UPLOAD_SUB_PATH, image.filename));

    const sitter = await this.sitterService.create(body.description, body.price, body.images, selectedUser);
    const sitterDto = plainToClass(SitterDto, sitter, { groups: ['petman-api'] });
  ...
  @Get(':id')
  async get(@Param('id') id: string, @SelectedUserParam() selectedUser: User, @SitterParam() sitter: Sitter): Promise<SitterDto> {
  ...
  async update(
    ...
    await this.sitterService.update(sitter, body.description, body.price, body.images);
    const sitterDto = plainToClass(SitterDto, sitter, { groups: ['petman-api'] });
    sitterDto.isOwner = true;
  ...
  @UseGuards(AuthGuard, SitterExistsGuard, SitterOwnerGuard)
  @Delete(':id')
  async delete(@Param('id') id: string, @SitterParam() sitter: Sitter, @Res() res): Promise<void> {
  ...
  @Get('/')
  async list(@Query() query: ListQueryRequestDto, @SelectedUserParam() selectedUser: User): Promise<SitterListResponseDto> {
    const groups = ['petman-api'];
    if (!selectedUser) {
      groups.push('petman-unauthorised');
    }
